...
def calculate_BDT_input_numbers(mode_names, sig, df, eff, xsec, frac):
    ...
    f2 = open(pkl_path+"xsec_process.txt","w")
    print(f"Calculating number of BDT inputs for {mode_names}")
    print(f"eff = {eff}")
    print(f"xsec = {xsec}")
    print(f"Calculating number of BDT inputs for {mode_names}",file=f2)
    print(f"eff = {eff}",file=f2)
    print(f"xsec = {xsec}",file=f2)
...
def run(modes, n_folds, stage):
    ...
    #procFile = "/afs/cern.ch/user/k/kdewyspe/21_March/FCCeePhysicsPerformance/case-studies/higgs/mH-recoil/FCCAnalyses-config/TopHiggs/first_look/FCCee_procDict_winter2023_IDEA.json"
    ...
    pkl_path = loc.PKL if stage == "training" else loc.PKL_Val
    f = open(pkl_path+"output_process.txt","w")
    ...
    for cur_mode in mode_names:
        ...
        print(f"Number of events in {cur_mode} = {N_events[cur_mode]}", file=f)
        print(f"Efficiency of {cur_mode} = {eff[cur_mode]}", file=f)
    ...
    print(f"Number of BDT inputs = {N_BDT_inputs}" ,file=f)
    for cur_mode in mode_names:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--Mode", action="store", dest="modes", default=["mumuH", "egamma", "gammae" ], help="Decay mode")
    ...
    run(**args) #, "ZZ", "WWmumu", "Zll", , "gaga_mumu"